    ...
    <div class="container">
        ...
        <form method="POST" action="{{ route('admin.pegawai.update', $pegawai->id) }}">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="nomor_induk" name="nomor_induk" value="{{ $pegawai->nomor_induk }}" required>
            </div>
            ...
            <div class="form-group">
                ...
                <select class="form-select" id="status" name="status" aria-label="Default select example" required>
                    ...
                    @foreach (\App\Models\Pegawai::getStatusOptions() as $statusValue => $statusLabel)
                        <option value="{{ $statusValue }}" {{ $pegawai->status == $statusValue ? 'selected' : '' }}>{{ $statusLabel }}</option>
                    @endforeach
                </select>
            </div>            
            ...
            <div class="form-group">
                ...
                <select class="form-select" id="unit_id" name="unit_id" aria-label="Default select example" required>
                    ...
                    @foreach ($units as $unit)
                        <option value="{{ $unit->id }}" {{ $pegawai->unit_id == $unit->id ? 'selected' : '' }}>{{ $unit->nama }} ({{ $unit->singkatan }})</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <select class="form-select" id="jenis" name="jenis" aria-label="Default select example" required>
                    ...
                    @foreach (\App\Models\Pegawai::getJenisOptions() as $jenisValue => $jenisLabel)
                        <option value="{{ $jenisValue }}" {{ $pegawai->jenis == $jenisValue ? 'selected' : '' }}>{{ $jenisLabel }}</option>
                    @endforeach
                </select>
            </div>            
            ...
            <div class="form-group">
                ...
                <select class="form-select" id="user_id" name="user_id" aria-label="Default select example" required>
                    ...
                    @foreach ($users as $user)
                        <option value="{{ $user->id }}" {{ $pegawai->user_id == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                    @endforeach
                </select>
            </div>            
